...
import 'package:face_detector_getx/utils_scanner.dart';
...
class _HomeState extends State<Home> {
  ...
  initCamera()async {
    ...
    await cameraController!.initialize().then((value)
    {
      ...
      cameraController!.startImageStream((imageFromStream) => {
        if(!isWorking) {
          isWorking = true,
          performDetactionOnStreamFrames(imageFromStream),
        }
      });
    });
  }
  ...
  dynamic scanResults;
  performDetactionOnStreamFrames(CameraImage cameraImage) async
  {
    UtilsScanner.detect(
      image: cameraImage,
